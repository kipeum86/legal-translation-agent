...
설정은 `${LEGAL_TRANSLATION_PRIVATE_DIR}/config.json`에 저장됩니다. `/setup`으로 언제든 재설정 가능합니다.
...
| **Normal** | 전략 분화 이중 패스 → 종합 → 구조 검증 | ~2.5배 | 일반 문서 — 기본값 |
| **Hard** | Normal + 역번역 + Library 비교 + 편집 교정 | ~5-6배 | 출판 수준, 고위험 번역 |

### Fast 모드 초안 흐름 (Step 1~5)

Fast 모드는 문서 분석, 용어 설정, 단일 번역 초안, 결정론적 구조/용어 검증, 초안 출력 조립만 실행합니다. 패스 B와 비교 종합을 생략하므로 결과물은 초안/내부 검토용으로 표시되어야 합니다.
...
1. **계획**: 로컬 스크립트와 LLM 동시성 한도를 포함한 dry-run 배치 계획 생성
2. **Phase 1**: 모든 문서를 병렬로 파싱, 구조 카운트, 용어 후보 추출
3. **검토**: `batch-glossary-review.json` 승인; 당사자명과 정의 용어를 번역 전에 batch-level lock으로 고정
4. **Phase 3**: 승인된 batch-level lock을 사용해 문서별 번역을 병렬 실행
5. **문서 간 일관성 검사**: 당사자명, 정의 용어, 날짜 형식, 법률 참조 표현을 문서 전체에서 확인
6. **최종 용어집 병합**: 모든 새 용어 저장
...
빠른 내부 초안이 필요하고 전문가가 다시 검토할 때만 Fast 모드를 사용하세요.
